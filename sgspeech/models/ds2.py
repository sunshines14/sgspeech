...
class ConvModule(tf.keras.Model):
    def __init__(self,
        ...
        self.blocks = [
            ConvBlock(conv_type=conv_type, kernels=kernels[i], strides=strides[i], filters=filters[i], dropout=dropout,
                      name=f"{self.name}_block_{i}") for i in range(len(filters))
        ]

        self.postprocess = None
        if conv_type == "conv2d": self.postprocess = Reshape(name=f"{self.name}_preprocess")
