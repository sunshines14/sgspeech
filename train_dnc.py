...
from sgspeech.models.dnc import DNC
...
with ctc_trainer.strategy.scope():
    ds2_model = DNC(**config.model_config, output_size=text_featurizer.num_classes)
    #tf.print(f"dddd!: {speech_featurizer.shape}")
    ds2_model._build(speech_featurizer.shape)
    ds2_model.summary(line_length=120)
# Compile
ctc_trainer.compile(DNC, config.learning_config.optimizer_config,
